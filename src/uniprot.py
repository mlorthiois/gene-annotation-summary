...
def uniprot(gene, organism, output_file):
    ...
    uniprot_name = []
    r = requests.get(
        "https://www.uniprot.org/uniprot/?query=gene_exact%3A{}+organism%3A{}&columns=id,protein_names&format=tab".format(gene, organism))
    ...
    if result != []:
        ...
        for id in uniprot_id:
            output_file.write(
                '<a href="https://www.uniprot.org/uniprot/{}">{}</a><br>'.format(id, id))
        output_file.write("</td><td><div class='scroll'>" +
                          "<br>".join(uniprot_name)+"</div></td>")
        return uniprot_id
    else:
